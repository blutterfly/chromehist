...
# Define regular expression pattern for 12-character ID filtering
id_pattern = re.compile(r'-[a-f1-9]{12}')
...
with open('filtered_chrome_history.csv', 'w', newline='', encoding='utf-8') as f:
    ...
    for row in cursor.fetchall():
        ...
        if re.search(id_pattern, url) and 'medium' in url.lower() and 'source=explore' not in url.lower():
